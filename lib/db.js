...
async function initDb() {
  ...
  await getPool().query(`
    ...
    ADD COLUMN IF NOT EXISTS roblox_display_name TEXT NOT NULL DEFAULT '';
    ...
    UPDATE tournaments
    SET status = CASE
      WHEN LOWER(status) IN ('signup', 'open') THEN 'open'
      WHEN LOWER(status) IN ('closed', 'running', 'ongoing') THEN 'ongoing'
      WHEN LOWER(status) IN ('completed', 'cancelled', 'ended') THEN 'ended'
      ELSE 'open'
    END
